...
/*
Time complexity: O(n^2). Recursion equation: T(n) = 4T(n/4) + n^2. Each recursion will divide the grid array into 4 parts, and each part will cost at most n^2 to check whether the elements in the restricted range in the array are all the same.

Space complexity: if not count the return result, then it's O(n). Because it may create at most n stacks during runtime. If count the return result, then it should be O(n^2).
*/
class Solution {
